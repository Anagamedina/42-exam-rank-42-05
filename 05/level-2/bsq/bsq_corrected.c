...
void process_file(char *filename)
{
    ...
    // Parsear metadatos manualmente
    int rows = 0;
    int pos = 0;
    while (line[pos] >= '0' && line[pos] <= '9') {
        rows = rows * 10 + (line[pos] - '0');
        pos++;
    }
    ...
    // Extraer caracteres manualmente
    pos++; // Saltar espacio
    if (line[pos] != '\0') empty = line[pos];
    pos += 2; // Saltar espacio y carácter
    if (line[pos] != '\0') obstacle = line[pos];
    pos += 2; // Saltar espacio y carácter
    if (line[pos] != '\0') full = line[pos];
    ...
    for (int i = 0; i < rows; i++) {
        ...
        // Copia manual en lugar de strcpy
        int j = 0;
        while (line[j] != '\0') {
            map[i][j] = line[j];
            j++;
